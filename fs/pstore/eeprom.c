 ...
 * Copyright (C) 2018 Jookia <166291@gmail.com>
 *
...
static ssize_t eeprom_pstore_read(struct pstore_record *record)
{
	struct eeprom_priv *priv = record->psi->data;
	struct eeprom_bucket bucket;
	int err;
...
static int notrace eeprom_pstore_write(struct pstore_record *record)
{
	enum pstore_type_id type = record->type;
	enum kmsg_dump_reason reason = record->reason;
	struct eeprom_priv *priv = record->psi->data;
	struct eeprom_bucket bucket;
	int ret;

	dev_dbg(&priv->i2c->dev, "Considering writing record %d/%d part %d, %d bytes...\n",
			type, reason, record->part, record->size);
...
//	}
	...
	bucket.header.size = record->size;
	bucket.data = (char *)record->buf;
...
static int eeprom_pstore_erase(struct pstore_record *record)
{
	struct eeprom_priv *priv = record->psi->data;
	struct i2c_client *i2c = priv->i2c;
	ulong offset;
	...
	offset = id_to_offset(priv, record->id);
